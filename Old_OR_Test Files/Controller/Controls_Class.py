...
class Rover_Controls:
    ...
    def Motor_PWM(self):
        ...
        right_side_motors = self.maximum_voltage*right_t*(1.0-max(0, (right_x-self.dead_zone)*(1/(1-self.dead_zone)))) + self.upper_loss
        left_side_motors = self.maximum_voltage*right_t*(1.0+min(0, (right_x+self.dead_zone)*(1/(1-self.dead_zone)))) + self.upper_loss

        return [int(left_side_motors), int(right_side_motors)]
