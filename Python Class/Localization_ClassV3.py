...
class localization:
    ...
    def calibrate_first_marker(self, x, y, z, rVx, rVy, rVz, tags_ids, marker_id):
        if self.sample_count < self.samples:
            ...
            if closest_marker_index != -1:
                self.x_samples.append(x[closest_marker_index])
                self.y_samples.append(y[closest_marker_index])
                self.z_samples.append(z[closest_marker_index])
                self.rx_samples.append(rVx[closest_marker_index])
                self.ry_samples.append(rVy[closest_marker_index])
                self.rz_samples.append(rVz[closest_marker_index])
                self.sample_count += 1

        else:
            # Calculate the average values for x, y, and z
            self.avg_x = np.median(self.x_samples)
            self.avg_y = np.median(self.y_samples)
            self.avg_z = np.median(self.z_samples)

            self.avg_rx = np.median(self.rx_samples)
            self.avg_ry = np.median(self.ry_samples)
            self.avg_rz = np.median(self.rz_samples)

            print(f"Calibration completed for marker {marker_id}")
            print(f"Average x: {self.avg_x}, Average y: {self.avg_y}, Average z: {self.avg_z}")
            self.calibration_bool = False

            return self.avg_x, self.avg_y, self.avg_z, self.avg_rx, self.avg_ry, self.avg_rz
        return 0, 0, 0, 0, 0, 0
    
    def get_origin_tag(self, tags_ids, dist, x, y, z, rVx, rVy, rVz):
            ...
            if(tags_ids is not None):
                if len(tags_ids) > 0:
                    ...
                    if(self.index is None):
                        self.index = np.argmin(dist)   # get closest one
                        self.closest_marker_id = tags_ids[self.index]  # take the tag id and filter it
                        ...
                        print(f"Calibrating marker with ID: {self.closest_marker_id}")
            
                        # Create lists to store 30 samples for x, y, and z
                        self.x_samples, self.y_samples, self.z_samples = [], [], []
                        self.rx_samples, self.ry_samples, self.rz_samples = [], [], []

                        # Collect 30 samples of x, y, and z for the specified marker
                        self.sample_count = 0

                    # Call calibration function for the closest marker
                    _, _, _, drx, dry, drz = self.calibrate_first_marker(x, y, z, rVx, rVy, rVz, tags_ids, self.closest_marker_id)

                    if not self.calibration_bool:
                        # set that in the coordinates
                        self.actual_tag = np.append(self.actual_tag, self.closest_marker_id)
    ...
    def update_pygames_screen(self):
        ...
        self.clock.tick(60)
        # end program 
        return self.end_program, img_tk
    ...
    def controller_handler(self, cal, up, down):
        ...
        if(up):
            if(self.zoom_factor < 100*self.zoom_step):
                self.zoom_factor += self.zoom_step
        elif(down):
            if(self.zoom_factor > 1*self.zoom_step):
                self.zoom_factor -= self.zoom_step