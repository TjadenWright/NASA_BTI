...
class GUI:
    ...
    def debug(self):
        ...
        if(self.debugger == 1):
            self.button1a.config(text="Stop Debugging")
        else:
            self.button1a.config(text="Debugging")
    ...
    def set_up_Main_UI(self, battery):
        ...
        self.button1a = Button(self.frame1, text="Debugging", bg="#FFD100", fg="black", command=self.debug)
    ...
    def loop_Main_UI(self, local_img):
        ...
        if time.time() - self.last_update_time >= 1 or self.prev_bms is not self.bms_numb or self.first:
            ...
            self.speedometer(self.screenDiag, round(self.total_voltage[self.bms_numb], 1), "V", "Battery Voltage", 60, BATTERY_X + BATTERY_WIDTH + self.steps, self.down_step + BATTERY_HEIGHT // 2)
            self.speedometer(self.screenDiag, round(self.current[self.bms_numb], 1), "A", "Battery Current", 100, BATTERY_X + BATTERY_WIDTH + 2*self.steps, self.down_step + BATTERY_HEIGHT // 2)
            self.speedometer(self.screenDiag, round(self.power[self.bms_numb], 1), "W", "Battery Power", 100, BATTERY_X + BATTERY_WIDTH + 3*self.steps, self.down_step + BATTERY_HEIGHT // 2)
            self.speedometer(self.screenDiag, round(self.capacity_remaining[self.bms_numb], 1), "Ah", "Battery Capacity", 100, BATTERY_X + BATTERY_WIDTH + 4*self.steps, self.down_step + BATTERY_HEIGHT // 2)
