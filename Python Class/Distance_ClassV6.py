...
class aruco_detect:
    ...
    # get the tag data
    def aruco_tags(self, pic_out=True, FPS_read=False, Frame=None):
        ...
        if Frame is not None:
            ...
            if pic_out:
                ...
                cv2.putText(display_frame, f"Frame Rate: {self.smoothed_frame_rate:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                cv2.imshow('Video', display_frame)
            elif FPS_read:
                print(f"Frame Rate: {self.smoothed_frame_rate:.2f}")
            else:
                img1 = cv2.cvtColor(Frame, cv2.COLOR_BGR2RGB)
                img2 = cv2.resize(img1, self.Output_Res)
                self.img = ImageTk.PhotoImage(Image.fromarray(img2))
        else:
            self.x_values = None
            self.y_values = None
            self.z_values = None
